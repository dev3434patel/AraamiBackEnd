...
class Updator{
    ...
    public function language_generate(){
        ...
        // start user blade language
        $user_custom_language = [];
        $user_new_languages = include('lang/en/admin.php');

        $user_existing_languages = include('../lang/en/admin.php');

        foreach($user_existing_languages as $user_exist_key => $user_exist_language){
            $user_custom_language[$user_exist_key] = $user_exist_language;
        }

        foreach($user_new_languages as $user_new_key => $user_new_language){
            $user_custom_language[$user_new_key] = $user_new_language;
        }

        file_put_contents('../lang/en/admin.php', "");
        $user_custom_language = var_export($user_custom_language, true);
        file_put_contents('../lang/en/admin.php', "<?php\n return {$user_custom_language};\n ?>");

        // end user blade language



        // start user validation language
        $user_valid_custom_language = [];
        $user_valid_new_languages = include('lang/en/admin_validation.php');

        $user_valid_existing_languages = include('../lang/en/admin_validation.php');

        foreach($user_valid_existing_languages as $user_valid_exist_key => $user_valid_exist_language){
            $user_valid_custom_language[$user_valid_exist_key] = $user_valid_exist_language;
        }

        foreach($user_valid_new_languages as $user_valid_new_key => $user_valid_new_language){
            $user_valid_custom_language[$user_valid_new_key] = $user_valid_new_language;
        }

        file_put_contents('../lang/en/admin_validation.php', "");
        $user_valid_custom_language = var_export($user_valid_custom_language, true);
        file_put_contents('../lang/en/admin_validation.php', "<?php\n return {$user_valid_custom_language};\n ?>");

        // end user validation language
